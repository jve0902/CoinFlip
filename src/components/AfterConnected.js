...
const AfterConnected = () => {
  ...
  const coinOptions = ["thumbs up", "perfect"];
  const solOptions = ["5", "20", "50", "100", "150", "250", "500"];
  ...
  return (
    <div>
      ...
      <div className="flex justify-center flex-col items-center">
        ...
        <h2 className="font-bold text-4xl mt-4 text-dark rounded-lg p-3">
          {showResult ? `Result : ${result}` : "Whats your vibe?"}
        </h2>
        <div className="grid grid-flow-col justify-center gap-3 mt-8">
          {coinOptions.map((value, key) => (
            <button
              ...
              key={key}
              className={` rounded-md uppercase py-2 px-8 border-gray-600 border-[1px] font-bold text-sm md:text-xl shadow-lg ${currentCoinOption === key ? "bg-yellow-700 px-10 py-5 font-bold text-white rounded rounded-tr-3xl rounded-bl-3xl" : 'bg-slate-500 py-5 font-bold text-white rounded rounded-tr-3xl rounded-bl-3xl'
                }`}
            >
              {value}
            </button>
        ...
        <div className="flex  flex-wrap grid-flow-col justify-center gap-3 mt-4">
          {solOptions.map((value, key) => (
            <button
              ...
              key={key}
              className={` uppercase rounded-md py-2 px-4 border-gray-600 border-[1px] font-bold text-sm md:text-xl shadow-lg ${currentSolOption === key ? "bg-yellow-400" : "bg-transparent"
                }`}
            >
              {value}&nbsp;HBAR
            </button>
